...
import com.zhl.CBPullRefresh.CBRefreshFooter;
...
public class PullToRefreshSwipeMenuListView extends ListView implements OnScrollListener {
    ...
    // -- header view
    private CBRefreshHeader mHeaderView;
    ...
    private final static int PULL_LOAD_MORE_DELTA = 120;
    private final static float OFFSET_RADIO = 2.3f;
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if ((mTouchView == null || !mTouchView.isActive()) && Math.pow(dx, 2) / Math.pow(dy, 2) <= 3) {
                    if (getFirstVisiblePosition() == 0 && (mHeaderView.getVisiableHeight() > 0 || deltaY > 0)) {
                        ...
                        invokeOnScrolling();
                    } else if (getLastVisiblePosition() == mTotalItemCount - 1 && (mFooterView.getLoadMorePullUpDistance() > 0 || deltaY < 0)) {
...
//                } else
                if (getFirstVisiblePosition() == 0) {
                    ...
                    if (mEnablePullRefresh && mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
                        ...
                        mHeaderView.setState(CBRefreshHeader.STATE_REFRESHING);
